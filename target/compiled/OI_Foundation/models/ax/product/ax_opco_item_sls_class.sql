...
with ax_opco_item_sls_class as (
    ...
    from FIVETRAN.AX_PRODREPLICATION1_DBO.INVENTSALESCLASS_OPI
...
final as(
    ...
    varchar
), '') || '-' || coalesce(cast(src_item_sls_class_cd as 
    varchar
), '') as 
    varchar
)) as opco_item_sls_class_sk, 
            md5(cast(coalesce(cast(src_sys_nm as 
    varchar
), '') || '-' || coalesce(cast(src_item_sls_class_cd as 
    varchar
), '') || '-' || coalesce(cast(src_item_sls_class_desc as 
    varchar
), '') as 
    varchar
)) as OPCO_ITEM_SLS_CLASS_ck,
            * 
...
select * from final
        ...
        where stg_load_dtm > (select max(stg_load_dtm) from OI_DATA_DEV_V2.intermediate_fnd_bkp.ax_opco_item_sls_class ) 
