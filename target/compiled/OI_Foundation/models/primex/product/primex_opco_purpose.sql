...
delete_captured as(
    select * from final
        ...
        from OI_DATA_DEV_V2.FND_DEV_BKP.OPCO_PURPOSE
...
select * from delete_captured
        ...
        where 
            (stg_load_dtm > (select max(stg_load_dtm) from OI_DATA_DEV_V2.intermediate_fnd_bkp.primex_opco_purpose ) and OPCO_PURPOSE_ck not in (select distinct OPCO_PURPOSE_ck from OI_DATA_DEV_V2.FND_DEV_BKP.OPCO_PURPOSE where src_sys_nm = 'SYSPRO-PRMX')) 
            or 
            (stg_load_dtm > (select max(stg_load_dtm) from OI_DATA_DEV_V2.intermediate_fnd_bkp.primex_opco_purpose ) and OPCO_PURPOSE_ck in (select distinct OPCO_PURPOSE_ck from OI_DATA_DEV_V2.FND_DEV_BKP.OPCO_PURPOSE where src_sys_nm = 'SYSPRO-PRMX') and delete_dtm is not null)
