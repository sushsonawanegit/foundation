...
select * from v_jde_opco_sub_ledger_type
    ...
    select 
    md5(cast(coalesce(cast('JDE-HNCK' as 
    varchar
), '') || '-' || coalesce(cast(f0902.gbco as 
    varchar
), '') || '-' || coalesce(cast(f0902.gbmcu as 
    varchar
), '') || '-' || coalesce(cast(f0902.gbobj as 
    varchar
), '') || '-' || coalesce(cast(f0902.gbsub as 
    varchar
), '') || '-' || coalesce(cast(f0902.gbsblt as 
    varchar
), '') || '-' || coalesce(cast(f0902.gbsbl as 
    varchar
), '') || '-' || coalesce(cast(f0902.gbctry as 
    varchar
), '') || '-' || coalesce(cast(f0902.gbfy as 
    varchar
), '') as 
    varchar
    ...
    iff(trim(f0902.gblt) = 'AU', trim(f0902.gbapyc)/(select pow(10, trim(frcdec)::number) from MULE_STAGING.JDE_HANCOCK.JDE920_DD920_F9210 where trim(frdtai) = 'APYC'), null) as opening_bal_qty,
    iff(trim(f0902.gblt) = 'AA', trim(f0902.gbapyc)/(select pow(10, trim(frcdec)::number) from MULE_STAGING.JDE_HANCOCK.JDE920_DD920_F9210 where trim(frdtai) = 'APYC'), null) as trans_currency_opening_bal_amt,
    iff(trim(f0902.gblt) = 'AA', trim(f0902.gbapyc)/(select pow(10, trim(frcdec)::number) from MULE_STAGING.JDE_HANCOCK.JDE920_DD920_F9210 where trim(frdtai) = 'APYC'), null) as opco_currency_opening_bal_amt,
    null as opco_uom_sk,
    null as opco_brand_sk,
    oslt.opco_sub_ledger_type_sk,
    trim(f0902.gbsbl) as sub_ledger_cd
    from MULE_STAGING.JDE_HANCOCK.JDE_PRODUCTION_PRODDTA_F0902 f0902
...
select * from delete_captured
        ...
        where 
            (stg_load_dtm > (select max(stg_load_dtm) from OI_DATA_DEV_V2.intermediate_fnd_bkp.jde_opco_fscl_yr_gl_opening_bal ) and OPCO_FSCL_YR_GL_OPENING_BAL_ck not in (select distinct OPCO_FSCL_YR_GL_OPENING_BAL_ck from OI_DATA_DEV_V2.FND_DEV_BKP.OPCO_FSCL_YR_GL_OPENING_BAL where src_sys_nm = 'JDE-HNCK')) 
            or 
            (stg_load_dtm > (select max(stg_load_dtm) from OI_DATA_DEV_V2.intermediate_fnd_bkp.jde_opco_fscl_yr_gl_opening_bal ) and OPCO_FSCL_YR_GL_OPENING_BAL_ck in (select distinct OPCO_FSCL_YR_GL_OPENING_BAL_ck from OI_DATA_DEV_V2.FND_DEV_BKP.OPCO_FSCL_YR_GL_OPENING_BAL where src_sys_nm = 'JDE-HNCK') and delete_dtm is not null)
