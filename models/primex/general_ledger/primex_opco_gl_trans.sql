-- depends_on: {{ ref('opco') }}
-- depends_on: {{ ref('opco_chart_of_accts') }}
-- depends_on: {{ ref('opco_gl_trans_type') }}
-- depends_on: {{ ref('opco_cost_center') }}
-- depends_on: {{ ref('opco_type') }}
-- depends_on: {{ ref('opco_dept') }}
-- depends_on: {{ ref('opco_purpose') }}
-- depends_on: {{ ref('opco_lob') }}
-- depends_on: {{ ref('opco_uom') }}
-- depends_on: {{ ref('opco_sub_ledger_type') }}
-- depends_on: {{ ref('opco_currency') }}
...
with primex_opco_gl_trans as(
    ...
    {% for tbl in tables %}
        ...
        op.opco_purpose_sk,
        ol.opco_lob_sk,
        ifnull(oc.opco_currency_sk, (select opco_currency_sk from {{ ref('opco')}} where src_sys_nm = 'SYSPRO-PRMX' and opco_id = '{{ tbl}}')) as trans_currency_sk,
        ...
        left join {{ ref('opco_gl_trans_type')}} ogtt 
        ...
        left join {{ ref('opco_sub_ledger_type')}} oslt 
