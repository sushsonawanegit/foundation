{% set _load = load_type('JDE_HNCK_OPCO_GL_TRANS') %}

with grouped_data as(
    select 
    'JDE-HNCK' as src_sys_nm,
    max(f0911.mule_load_ts) as stg_load_dtm,
    ...
    sum(trim(f0911.glu))/(select pow(10, frcdec::number) from {{ source('JDE_HNCK_DEV3', 'F9210') }} where trim(frdtai) = 'U') as gl_qty,
    sum(iff(trim(f0911.glacr) = 0 or trim(f0911.glacr) = trim(f0911.glaa), trim(f0911.glaa), trim(f0911.glacr)))/(select pow(10, frcdec::number) from {{ source('JDE_HNCK_DEV3', 'F9210') }} where trim(frdtai) = 'AA') as trans_currency_gl_amt,
    sum(trim(f0911.glaa))/(select pow(10, frcdec::number) from {{ source('JDE_HNCK_DEV3', 'F9210') }} where trim(frdtai) = 'AA') as opco_currency_gl_amt
    from {{ source('JDE_HNCK_DEV1', 'F0911') }}  f0911 
...
jde_hnck_opco_gl_trans as (
    ...
    left join {{ ref('jde_hnck_opco_gl_trans_type_curr')}} ogtt 
...
delete_captured as(
    select * from final
    {% if tb_check(var('fnd_tbl_db'), var('intermediate_tbl_sch'), 'JDE_HNCK_OPCO_GL_TRANS') and tb_check(var('fnd_tbl_db'), var('fnd_tbl_sch'), 'OPCO_GL_TRANS') and _load != 3%}
        union
        select
